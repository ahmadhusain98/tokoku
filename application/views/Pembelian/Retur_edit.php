...
$tgl_retur = date("Y-m-d", strtotime($header->tgl_retur));
$jam_retur = date("H:i:s", strtotime($header->jam_retur));
...
if ($header->invoice_terima == "" || $header->invoice_terima == null) {
    $data_terima = "";
    $cekpo = "";
    $cek_po = 0;
} else {
    $data_terima = $this->db->query("SELECT invoice AS id, concat(invoice, ' | ', tgl_terima, ' | ', jam_terima) AS text FROM pembelian_h WHERE invoice = '$header->invoice_terima'")->row();
    $cekpo = "checked";
    $cek_po = 1;
}
if ($header->ppn > 0) {
                    ...
                    <div class="col-sm-12">
                        <div class="table-responsive">
                                        ...
                                        <div class="row mb-3">
                                            ...
                                            <div class="col-sm-6">
                                                <div class="row">
                                                    ...
                                                    <div class="col-sm-9">
                                                        <select name="kode_terima" id="kode_terima" class="form-control select2_penerimaan" data-placeholder="Penerimaan Barang" onchange="get_data_penerimaan(this.value)" style="width: 100%;">
                                                            <?php if ($data_terima) : ?>
                                                                <option value="<?= $data_terima->id; ?>"><?= $data_terima->text; ?></option>
                                                            <?php endif; ?>
                                                        </select>
                                                    </div>
                                                </div>
                                                    ...
                                                    <div class="col-sm-9">
                                                        ...
                                                        <div class="row">
                                                            <div class="col-sm-6" style="margin-bottom: 5px;">
                                                                <input type="date" id="tgl_retur" name="tgl_retur" class="form-control" value="<?= $tgl_retur; ?>" <?= $dis; ?>>
                                                            </div>
                                                            <div class="col-sm-6">
                                                                <input type="time" id="jam_retur" name="jam_retur" class="form-control" value="<?= $jam_retur; ?>" <?= $dis; ?>>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                            ...
                            <div class="card shadow border-danger">
                                <div class="card-header">
                                    <h3 class="card-title">Detail Data Retur</h3>
                                    <div class="card-tools">
                                        <button type="button" class="btn btn-tool" data-card-widget="collapse" title="Collapse">
                                            <i class="fas fa-minus"></i>
                                        </button>
                                                    ...
                                                    <tbody id="datatable_body">
                                                        ...
                                                        ?>
                                                            <tr id="tr_<?= $no; ?>">
                                                                ...
                                                                <td>
                                                                    <input type="text" name="qty_barang[]" id="qty_barang<?= $no; ?>" class="form-control text-right" onchange="total_qty(<?= $no; ?>)" value="<?= number_format($d->qty); ?>" min="1">
                                                                </td>
                                                                <td>
...
<script>
    function simpan() {
        ...
        $.ajax({
            url: "<?= site_url('Pembelian/simpan_retur/2') ?>",
            type: "POST",
            ...
            success: function(data) {
                if (data.status == 1) {
                    Swal.fire({
                        icon: 'success',
                        html: '<span class="text-success h4 font-weight-bold">UBAH DATA RETUR</span><br><b>' + (data.invoice).toUpperCase() + '</b><br><br>Berhasil diubah!',
                    }).then((result) => {
                    ...
                    Swal.fire({
                        icon: 'error',
                        html: '<span class="text-danger h4 font-weight-bold">UBAH DATA RETUR</span><br><br>Gagal diubah!',
                    });
                }
            }
        })
